...
struct KanbanBoardView: View {
    ...
    private func goalsFor(category: GoalCategory) -> [Goal] {
        viewModel?.goalsFor(category: category) ?? []
    }
    ...
    var body: some View {
        ...
        .refreshable {
            ...
            viewModel?.loadAllGoals(searchText: searchText)
        }
        .sheet(isPresented: $showingNewGoalSheet) {
            GoalEditorSheet(goal: nil, category: newGoalCategory) { goal in
        ...
        .onAppear {
            if viewModel == nil {
                viewModel = KanbanViewModel(dataService: GoalDataService(modelContext: modelContext))
            }
            viewModel?.loadAllGoals(searchText: searchText)
        }
        .onChange(of: searchText) { _, newValue in
            viewModel?.loadAllGoals(searchText: newValue)
        }
    }

    private func moveGoal(_ goal: Goal, to category: GoalCategory) {
        withAnimation(.easeInOut(duration: AppConfiguration.UI.animationDuration)) {
            do {
                try viewModel?.moveGoal(goal, to: category)
            } catch {
...
struct KanbanCardView: View {
    ...
    private func toggleCompletion() {
        withAnimation {
            ...
            if !modelContext.saveWithErrorHandling() {
                ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.toggleCompletion")
            }
        }
    }
    ...
    private func moveToCategory(_ category: GoalCategory) {
        ...
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.moveToCategory")
        }
    }
    ...
    private func setPriority(_ priority: Priority) {
        ...
        goal.updatedAt = Date()
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.setPriority")
        }
    }
    ...
    private func toggleFocus() {
        ...
        goal.updatedAt = Date()
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.toggleFocus")
        }
    }
    ...
    private func archiveGoal() {
        goal.archive()
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.archiveGoal")
        }
    }
    ...
    private func deleteGoal() {
        ...
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.deleteFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "KanbanCardView.deleteGoal")
        }
    }
}
