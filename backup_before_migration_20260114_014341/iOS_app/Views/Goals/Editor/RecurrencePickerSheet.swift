...
struct RecurrencePickerSheet: View {
    ...
    private func saveRecurrence() {
        ...
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "RecurrencePickerSheet.save")
        } else {
            dismiss()
        }
    }
    ...
    private func removeRecurrence() {
        if let recurrence = goal.recurrence {
            ...
            goal.updatedAt = Date()
            if !modelContext.saveWithErrorHandling() {
                ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "RecurrencePickerSheet.removeRecurrence")
            }
        }
        dismiss()
    }
