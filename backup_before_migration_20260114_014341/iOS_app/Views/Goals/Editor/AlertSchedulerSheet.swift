...
struct AlertSchedulerSheet: View {
    ...
    private func addAlert() {
        ...
        if !modelContext.saveWithErrorHandling() {
            ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "AlertSchedulerSheet.save")
        } else {
            dismiss()
        }
    }
    ...
    private func deleteAlert(_ alert: GoalAlert) {
        ...
        goal.updatedAt = Date()
            if !modelContext.saveWithErrorHandling() {
                ErrorHandler.shared.handle(.saveFailed(NSError(domain: "WhatsNextiOS", code: -1)), context: "AlertSchedulerSheet.deleteAlert")
            }
        }
}
