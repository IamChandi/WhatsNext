...
final class NotificationService {
    ...
    private func scheduleMorningReminder() async {
        ...
        // Schedule separate notifications for each weekday (Monday-Friday)
        // Calendar weekdays: Monday=2, Tuesday=3, Wednesday=4, Thursday=5, Friday=6
        let weekdays = [2, 3, 4, 5, 6] // Monday through Friday
        
        for weekday in weekdays {
            var dateComponents = DateComponents()
            dateComponents.weekday = weekday
            dateComponents.hour = AppConfiguration.Notifications.morningHour
            ...
            let request = UNNotificationRequest(
                identifier: "daily-morning-reminder-\(weekday)",
                content: content,
                trigger: trigger
            )
            ...
            do {
                ...
                Logger.notifications.info("✅ Morning reminder scheduled for weekday \(weekday)")
            } catch {
                Logger.notifications.error("❌ Failed to schedule morning reminder for weekday \(weekday): \(error.localizedDescription)")
                ErrorHandler.shared.handle(.notificationFailed(error), context: "NotificationService.scheduleMorningReminder")
            }
        }
    }
    ...
    private func scheduleEveningReminder() async {
        ...
        for weekday in weekdays {
            var dateComponents = DateComponents()
            dateComponents.weekday = weekday
            dateComponents.hour = AppConfiguration.Notifications.eveningHour
            dateComponents.minute = 0
            ...
            let request = UNNotificationRequest(
                identifier: "daily-evening-reminder-\(weekday)",
                content: content,
                trigger: trigger
            )
            ...
            do {
                try await UNUserNotificationCenter.current().add(request)
                Logger.notifications.info("✅ Evening reminder scheduled for weekday \(weekday)")
            } catch {
                Logger.notifications.error("❌ Failed to schedule evening reminder for weekday \(weekday): \(error.localizedDescription)")
                ErrorHandler.shared.handle(.notificationFailed(error), context: "NotificationService.scheduleEveningReminder")
            }
        }
    }

    func cancelDailyReminders() {
        // Cancel all weekday reminders (Monday-Friday = weekdays 2-6)
        let weekdays = [2, 3, 4, 5, 6]
        var identifiers: [String] = []
        for weekday in weekdays {
            identifiers.append("daily-morning-reminder-\(weekday)")
            identifiers.append("daily-evening-reminder-\(weekday)")
        }
    ...
    func moveUnfinishedDailyGoalsToTomorrow(context: ModelContext) {
        ...
        do {
            ...
            ErrorHandler.shared.handle(.saveFailed(error), context: "NotificationService.moveUnfinishedDailyGoalsToTomorrow")
        }
    }
    ...
    func scheduleAlert(for goal: Goal, alert: GoalAlert) async {
        ...
        do {
            ...
            ErrorHandler.shared.handle(.notificationFailed(error), context: "NotificationService.scheduleAlert")
        }
    }
    ...
    func snoozeAlert(for goal: Goal, alert: GoalAlert, minutes: Int = AppConfiguration.Notifications.defaultSnoozeMinutes) async {
    ...
    func scheduleDueDateReminder(for goal: Goal) async {
        ...
        do {
            ...
            ErrorHandler.shared.handle(.notificationFailed(error), context: "NotificationService.scheduleDueDateReminder")
        }
    }
