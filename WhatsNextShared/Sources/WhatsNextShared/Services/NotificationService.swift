...
public struct NaturalLanguageParser {
    ...
    public static func parse(_ text: String) -> GoalConfig {
        var title = text
...
public final class NotificationService {
    ...
    public func scheduleDailyReminders() async {
    ...
    public func cancelDailyReminders() {
    ...
    public func moveUnfinishedDailyGoalsToTomorrow(context: ModelContext) {
    ...
    public func scheduleAlert(for goal: Goal, alert: GoalAlert) async {
        let hasPermission = await requestPermission()
        guard hasPermission else { return }
    ...
    public func cancelAllAlerts(for goalId: UUID) {
    ...
    public func rescheduleAlert(for goal: Goal, alert: GoalAlert, newDate: Date) async {
        // Cancel existing
    ...
    public func snoozeAlert(for goal: Goal, alert: GoalAlert, minutes: Int = AppConstants.Notifications.snoozeMinutes) async {
        let newDate = Date().addingTimeInterval(Double(minutes * 60))
        await rescheduleAlert(for: goal, alert: alert, newDate: newDate)
    }

    public func scheduleDueDateReminder(for goal: Goal) async {
